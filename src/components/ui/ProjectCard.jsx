...
const ProjectCard = ({project}) => {
    const {title, discription, image, technologies, metrics, demoUrl , githubUrl} = project;
  ...
  return (
    <div className='group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300'>
        <div className='relative h-64 overflow-hidden'>
            <img src={image} alt={title} className='w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 md:hover:scale-110'/>
            ...
            <div className='absolute bottom-4 right-4 flex items-center gap-3 '>
                {demoUrl && (
                    <a href={demoUrl} target='_blank' rel='noopener noreferrer' className='p-2.5 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:bg-primary/30 hover:border-primary/50 transition-all duration-300 scale-110' title='View Demo'>
        ...
        <div className='flex flex-wrap gap-'>
            {technologies.map((tech, index)=>(
                <span key={index} className='px-3 py-1 text-xs font-medium text-primary bg-primary/10 border border-primary/20 rounded-lg hover:bg-primary/20 mb-2 ml-4 transition-colors duration-200'>
                    {tech}
                </span>
            ))}
        </div>
        {metrics && (
            <div className='flex items-center gap-2 pt-3 ml-4 mb-2 border-t border-white/10'>
