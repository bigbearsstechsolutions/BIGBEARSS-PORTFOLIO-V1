...
const RadialBg = ({ variant='hero', gradiants = [] }) => {
  const variants = {
    hero:[
      {
        position: 'top-1 left-1 -translate-x-1/2',
  ...
  const activeGradiants = variant === 'custom' ? gradiants : variants[variant] || variants.hero;

  const genrateGradiant = (colors) => {
